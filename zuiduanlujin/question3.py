...
n = int(n)
m = int(m)
graph_forward = Graph(n, True)
graph_backward = Graph(n, True)
for _ in range(m):
	s_input = input()
	v, w, cost = s_input.split(' ')
	graph_forward.add_edge(int(v) - 1, int(w) - 1, int(cost))
	graph_backward.add_edge(int(w) - 1, int(v) - 1, int(cost))

dijkstra_forward = Dijkstra(graph_forward, 0)
days_forward = dijkstra_forward.dist_to
dijkstra_backward = Dijkstra(graph_backward, 0)
days_backward = dijkstra_backward.dist_to
print(sum(days_forward + days_backward))