...
class Dijkstra(object):
	...
	def run(self):
		while self.fringe.size() > 0:
			...
			for w in self.graph.get_adj(v):
				dist = 100/(100-self.graph.get_dist(v, w))
	...
	def get_shortest_cost(self, v):
		return 100*self.dist_to[v]
...
dijkstra = Dijkstra(graph, int(start) - 1)
cost = dijkstra.get_shortest_cost(int(target) - 1)
print(round(cost, 8))